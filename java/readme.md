...
```
Member클래스 : 멤버아이디, 닉네임
Board클래스 : 글번호, 제목, 내용, 글쓴이
회원관리클래스 : 멤버Map, 멤버기능
회원 가입한 Member들을 map에 넣어서 관리 (key:userId, value:Member 인스턴스)
회원저장
회원검색
회원삭제
...
```
딜러 : 카드덱을 섞고 플레이어에게 5장씩 나눠줌
플레이어 2명 : 딜러에게 받은 카드를 정렬된 순서로 보여줌
정렬 : 숫자 오름차순
숫자가 같으면 스페이드, 하트, 다이아몬드, 클로버 순서로 정렬
승패기준 : 카드 숫자 총합이 큰 플레이어가 승리
같으면 무승부
```
